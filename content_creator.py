...
class ContentCreationModel:
    ...
    def create_video(self, images, quote, output_path="output_video.mp4"):
        ...
        Args:
            ...
            quote (str): Text to overlay on the images
        ...
        try:
            ...
            from moviepy.video.tools.drawing import text_image
            ...
            # Add text to each image and convert to clips
            clips = []
            for img in images:
                # Add text overlay to image
                img_with_text = self.add_text_to_image(img, quote)
                
                # Convert PIL image to numpy array
                img_array = np.array(img_with_text)
                
                # Create a clip with the image
            ...
            for i in range(len(clips)-1):
                transitions.append(mpy.vfx.fadein(clips[i+1], 0.5))
            
            # Replace all clips except the first with transitions
            if transitions:
                final_clips = [clips[0]] + transitions
            else:
                final_clips = clips
            ...
            try:
                if os.path.exists("background_music.mp3"):
                    audio = mpy.AudioFileClip("background_music.mp3")
                    audio = audio.subclip(0, final_clip.duration)
                    audio = audio.volumex(0.3)  # Reduce volume
